...
const page = () => {
    ...
    const { data: restaurantData, isPending: Updating } = useGetRestaurantUser(session?.data?.user.id, session?.data?.user?.access_token);
    ...
    const { mutate, isPending } = useMutation({
        ...
        onSettled(data: any) {
            if (data.status === 200) {
                queryClient.invalidateQueries({ queryKey: ["restaurant", restaurantData?.restaurantId] });
